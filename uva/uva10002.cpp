...
int main(void){
    ...
    while(true){
        ...
        for (int i = 0; i < count; i++){
            double one, two;
            scanf("%lf %lf",&one,&two);
            arr.push_back(point(one,two));
        }
        arr = CH(arr);
        
        if (arr.size() == 3)
           arr.push_back(point(arr[0].x,arr[0].y));
        
        double totalx = 0.0;
        double totaly = 0.0;
        for (int i = 0; i < arr.size()-1; i++){
            double factor = arr[i].x * arr[i+1].y - arr[i+1].x * arr[i].y;
            totalx += (arr[i].x + arr[i+1].x) * factor;
            totaly += (arr[i].y + arr[i+1].y) * factor;
        }
        arr.pop_back();
        double ta = 6*area(arr);
        totalx = -totalx/ta;
        totaly = -totaly/ta;
        if (totalx > -0.0005 && totalx < EPS)
           totalx = 0;
        if (totaly > -0.0005 && totaly < EPS)
           totaly = 0;
        printf("%.3lf %.3lf\n",totalx,totaly);   
    }
    return 0;
